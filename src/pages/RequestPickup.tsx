  ...
  if (submitted) {
    return (
      <div className="max-w-md mx-auto text-center py-16 animate-fade-in">
        ...
        <div className="mt-3 inline-flex items-center gap-2 rounded-md bg-accent/10 px-3 py-1.5 text-sm text-accent">
          <Clock className="h-3.5 w-3.5" />
          Estimated pickup: {selectedSlot?.estimate || "TBD"}
        </div>
        <p className="text-xs text-muted-foreground mt-2">A worker will be assigned shortly.</p>
        {isEcommerce && (
        ...
        <Button className="mt-6" onClick={() => { setSubmitted(false); setWasteType(""); setTimeSlot(""); setIsEcommerce(false); }}>Submit Another</Button>
      </div>
    );
  }
      ...
      <form onSubmit={handleSubmit} className="space-y-4 rounded-lg border bg-card p-6">
        ...
        <div>
          ...
          {selectedSlot && (
            <p className="text-xs text-accent mt-2 flex items-center gap-1">
              ...
              Estimated pickup: {selectedSlot.estimate}
            </p>
          )}
        </div>
        ...
        <div className="flex items-center space-x-2 rounded-md border p-3">
          <Checkbox
            id="ecommerce"
            checked={isEcommerce}
            onCheckedChange={(checked) => setIsEcommerce(checked === true)}
          />
          <div className="grid gap-0.5 leading-none">
            <label htmlFor="ecommerce" className="text-sm font-medium cursor-pointer">
              E-commerce Packaging
            </label>
            <p className="text-xs text-muted-foreground">
              This waste is from online shopping deliveries
            </p>
          </div>
        </div>
